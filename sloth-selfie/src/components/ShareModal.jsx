...
const ShareModal = ({ openShareModal=null, setOpenShareModal }) => {
    ...
    const fullpath = `${window.location.origin}${openShareModal}`;
            ...
            <div className='modal-dialog modal-dialog-centered'>
                <div className='modal-content'>
                    <div className='modal-header'>
                        ...
                        <button type='button' className='btn-close' data-bs-dismiss='modal' aria-label='Close' onClick={() => {setOpenShareModal(null); setToggleClipboard(false)}}></button>
                    </div>
                    <div className='modal-body'>
                        <div className='d-flex flex-column gap-4 py-3 align-items-center justify-content-center'>
                            <div className=' d-flex flex-row gap-2 align-items-center justify-content-center'>
                                <label htmlFor='shareLink' className='m-0'>Link:</label>
                                <div className='input-group'>
                                    <input type='text' className='form-control' id='shareLink' value={fullpath} readOnly />
                                    <button className='input-group-text' onClick={() => handleCopy()}>
                                        {toggleClipboard ? <ClipboardCheck size={20} /> : <Clipboard size={20} />}
                                    </button>
                                </div>
                            </div>
