    ...
    const handleSubmit = async () => {
        if (selected.edit) {
            ...
            if (response.success){
                ...
                NewSwal.fire({ title: 'Activity edited', icon: 'success', text: 'Activity edited successfully'});
                console.log('Activity edited', activity);
                setActivities(activities.map(act => act._id === activity._id ? activity : act));
                resetActivity();
                setNotifications([]);
            } else NewSwal.fire({ title: 'Error editing activity', icon: 'error', text: response.message});
        } else {
            const response = await apiService(`/activity`, 'POST', activity);
            if (response.success){
                NewSwal.fire({ title: 'Activity added', icon: 'success', text: 'Activity added successfully'});
                setActivities([...activities, response.activity]);
                resetActivity();
            } else NewSwal.fire({ title: 'Error adding activity', icon: 'error', text: response.message});
            ...
            if (notifications.length > 0) {
                ...
                else NewSwal.fire({ title: 'Error adding notifications', icon: 'error', text: response.message});
            } else {
